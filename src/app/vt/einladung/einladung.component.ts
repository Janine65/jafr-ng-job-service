...
import { EINLADUNG_COLUMNS, EINLADUNG_EXAMPLE_SHEET } from './einladung.tables';
...
export class EinladungComponent implements OnInit, OnDestroy {
    private excelService = inject(ExcelService);
    private translate = inject(TranslateService);
    private jobService = inject(EinladungService);
    private logger = inject(LogFactoryService).createLogger('EinladungComponent');
    ...
    excelColumns = EINLADUNG_COLUMNS;

    // Progress list configuration
    progressListConfig = VT_DEFAULT_PROGRESS_CONFIG;

    ngOnInit(): void {
        this.logger.debug('Initializing EinladungComponent');
        ...
        this.logger.debug('EinladungComponent initialization complete');
    }

    ngOnDestroy(): void {
        this.logger.debug('Destroying EinladungComponent');
    ...
    async onFileSelected(file: File): Promise<void> {
        ...
        this.isLoading = true;
        // Show spinner immediately in running jobs list (no wait for polling)
    ...
    createExampleSheet(): void {
        ...
        this.exampleSheetHtml = this.excelService.createStyledPreview(EINLADUNG_EXAMPLE_SHEET, 'Einladung');
    }
    ...
    downloadExampleSheet(): void {
        ...
        this.excelService.downloadExcelFile(EINLADUNG_EXAMPLE_SHEET, 'einladung_beispiel', 'Einladung');
