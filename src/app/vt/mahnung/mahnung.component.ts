...
import { MAHNUNG_COLUMNS, MAHNUNG_EXAMPLE_SHEET } from './mahnung.tables';
...
@Component({
    ...
    templateUrl: './mahnung.component.html'
})
export class MahnungComponent implements OnInit, OnDestroy {
    private excelService = inject(ExcelService);
    private translate = inject(TranslateService);
    private jobService = inject(MahnungService);
    private logger = inject(LogFactoryService).createLogger('MahnungComponent');
    ...
    excelColumns = MAHNUNG_COLUMNS;

    // Progress list configuration
    progressListConfig = VT_DEFAULT_PROGRESS_CONFIG;

    ngOnInit(): void {
        this.logger.debug('Initializing MahnungComponent');
        ...
        this.logger.debug('MahnungComponent initialization complete');
    }

    ngOnDestroy(): void {
        this.logger.debug('Destroying MahnungComponent');
    }
    ...
    async onFileSelected(file: File): Promise<void> {
        ...
        this.isLoading = true;
        // Immediately show spinner in running jobs section so user sees activity without waiting for the next poll
    ...
    createExampleSheet(): void {
        ...
        this.exampleSheetHtml = this.excelService.createStyledPreview(MAHNUNG_EXAMPLE_SHEET, 'Mahnung');
    }
    ...
    downloadExampleSheet(): void {
        ...
        this.excelService.downloadExcelFile(MAHNUNG_EXAMPLE_SHEET, 'mahnung_beispiel', 'Mahnung');
    ...
    loadCompletedJobs(showLoading: boolean = false): void {
        ...
        // Ensure data is loaded (idempotent - only loads once if not already loaded)
