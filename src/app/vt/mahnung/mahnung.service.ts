...
import { MAHNUNG_REQUIRED_COLUMNS } from './mahnung.tables';

import type { JobDetailsResponse } from '@syrius/job-service';
import type { Mahnung } from './mahnung.model';

/**
 * Service configuration for Mahnung
 */
const MAHNUNG_CONFIG: JobServiceConfig = {
    serviceName: 'Mahnung',
    apiBasePath: '/bb/aktualisierung',
    endpointName: 'mahnung',
    searchEndpointName: 'searchBBAktualisierungMahnung',
    searchExcelFileEndpointName: 'searchExcelfile',
    translationPrefix: 'VT.BB.MAHNUNG',
    requiredColumns: MAHNUNG_REQUIRED_COLUMNS,
    rowTranslationKey: 'VT.BB.MAHNUNG.TABLE.ROW',
    taskName: 'reminder2'
};

/**
 * Service for Mahnung (BB Reminder/Warning) operations with backend API integration.
 *
 * Backend Integration:
 * 1. POST /uploadfile - Upload Excel file
 * 2. PUT /bb/aktualisierung/mahnung?filename=xxx - Trigger processing
 * 3. GET /bb/aktualisierung/searchBBAktualisierungMahnung?filename=xxx - Get entries/status
 */
@Injectable({
    providedIn: 'root'
})
export class MahnungService extends JobBaseService<Mahnung> {
    constructor() {
        super(MAHNUNG_CONFIG);
    }
    ...
    /**
     * Load job details by ID for Mahnung
     */
